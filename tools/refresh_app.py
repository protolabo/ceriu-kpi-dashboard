...
# A garder synchronisee avec le parametre VersionAPILinkedIn de Power BI et avec
# DEFAULT_LINKEDIN_API_VERSION dans connectors/linkedin.py. Si l'une des trois
# valeurs est mise a jour (ex: renouvellement annuel), mettre a jour les deux autres
# en meme temps pour eviter que cet outil utilise silencieusement une vieille version.
...
def linkedin_posts_with_stats(
    ...
    api_version : transmise au backend, qui l'utilise pour son appel a LinkedIn
    (header Linkedin-Version). Si non fournie ou vide, le backend retombe sur
    sa propre valeur par defaut.
    """
...
def run_refresh(mailchimp_key: str, facebook_token: str, facebook_page_id: str,
                 linkedin_token: str, linkedin_org_urn: str, linkedin_api_version: str,
    ...
    if linkedin_token:
        try:
            ...
            rows = linkedin_posts_with_stats(linkedin_token, linkedin_org_urn, linkedin_api_version)
...
def main():
    ...
    ttk.Label(frm, text="Version API LinkedIn (format AAAAMM) :").pack(anchor="w")
    linkedin_api_version_var = tk.StringVar(value=DEFAULT_LINKEDIN_API_VERSION)
    ttk.Entry(frm, textvariable=linkedin_api_version_var, width=45).pack(fill="x", pady=(0, 8))
    ...
    def start():
        ...
        linkedin_api_version = linkedin_api_version_var.get().strip()
        ...
        threading.Thread(
            ...
            args=(
                ...
                linkedin_token, linkedin_org_urn, linkedin_api_version,
